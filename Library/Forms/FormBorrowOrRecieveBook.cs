...
namespace Library
{
	public partial class FormBorrowOrRecieveBook : Form
	{
		...
		{
			if (DataGridViewForLendBook.CurrentCell.ColumnIndex == 0
				&& long.TryParse(DataGridViewForLendBook.CurrentCell.Value.ToString(), out long ID)) //TODO try catch null book/member
			{
				using LibraryContextForEFcore db = new();
